...
| Likely Quiz | Three predicted quizzes as tabs (lecture-only, lecture + book as the primary prediction, book-only), each in order with show/hide answers, a print stylesheet and "take this as a timed practice quiz" |
...
content/                       plain data, edited by content authors
  ...
  units/<unit-id>/             one folder per quiz or test
    unit.json  chapters/chNN.md  cross-reference.md  lexicon.json  questions.json  likely-quizzes.json
...
enum values, predicted-quiz ids that do not exist and a `likely-quizzes.json` that does not hold
exactly three quizzes with one primary, so broken content never reaches the live site.
...
(`quizConfig.ts`), the three predicted quizzes (`likelyQuizzes.ts`), spaced flashcards
(`spaced.ts`) and the content validator (`validate.ts`).
