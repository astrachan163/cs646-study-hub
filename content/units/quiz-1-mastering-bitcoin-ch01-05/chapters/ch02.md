...
**Lecture overlap:** this is the core of week 4. W4 s17 transaction anatomy ('Important info'); s18-23 Alice pays Bob: inputs reference unspent TXs, 'Bob accepts if (1) TX100 is unspent (2) Alice's signature is valid' (the 'most important two events'), multiple inputs/outputs, change ('pay yourself'), fee = inputs - outputs ('tip'; zero tip never mined); s24 m inputs / n outputs ('know what big vs small input/output mean'); s25 1 BTC = 100,000,000 satoshi; s26 block = header + last 10 minutes, Merkle tree, hash chain; s27-28 TX format (max 10,000 bytes); s29 UTXO database kept by miners; s32 SIGHASH; s33 TXID = double SHA256; s36, s42 gossip and P2P functions; s39 pool mining. Not covered in class: proof-of-work as a hash lottery (the book's term), the six-confirmation convention (only on a skipped slide), genesis block, offline signing, BIP21 invoices.
...
## Diagrams: transaction chain and block linking
...
_Source: Mastering Bitcoin, 3rd edition, chapter 2 (github.com/bitcoinbook/bitcoinbook, `develop` branch). Lecture citations `W4 s18` = week-4 slide 18. Full lecture-vs-book analysis: the Cross-Reference page._
